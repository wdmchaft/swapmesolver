...
NSArray *tokens = [string componentsSeparatedByString: @","];
		...
		Color color = [[tokens objectAtIndex:2] ColorEnumFromString];
		store(grid, x, y, color);
	}
}
-(void) display
{
...
(NSInteger) depth
{
	// base case
	if(depth == 0)
		return [grid count] == 0; // goal state
	...
	for(move in moves)
	{
		/*
		if(depth == 1
		   && ![move isEqual:[[MyMove alloc] initWithCellAndDir: [[Cell alloc] initWithXAndYAndColor: 2 : 2 : PINK]: LEFT]])
			continue;
		if(depth == 2
		 && ![move isEqual:[[MyMove alloc] initWithCellAndDir: [[Cell alloc] initWithXAndYAndColor: 3 : 1 : BLUE]: LEFT]])
		 continue;
		 */
		...
		success = [nextGrid iterate : (depth - 1)];
		if(success)
		{
			NSLog(@"%d: %@", depth, move);
			[nextGrid display];
